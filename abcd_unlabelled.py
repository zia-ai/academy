...
def main(input: str, unlabelled: str, sample: int, anonymize: bool, translation: str, source: str):
    process(input, unlabelled, sample, anonymize, translation, source)

def process(input: str, unlabelled: str, sample: int, anonymize: bool, translation: str, source: str):
    '''Process the file'''
    perf_log('Begin')

    # load data
    df = load_data_file(input)
    ...
    df = pandas.concat([df,pandas.DataFrame(df['original'].tolist(), columns=['abcd_role','utterance'])],axis=1)
    df['idx'] = df.groupby('abcd_id').cumcount()
    perf_log('Exploded original rows')

    # index the speakers
    df['idx_max'] = df.groupby(["abcd_id"])['idx'].transform(numpy.max)
    df['idx_customer'] = df.groupby(['abcd_id', 'abcd_role']).cumcount().where(df.abcd_role == 'customer', 0)
    df['idx_customer_max'] = df.groupby(["abcd_id"])['idx_customer'].transform(numpy.max)
    df['idx_agent'] = df.groupby(['abcd_id', 'abcd_role']).cumcount().where(df.abcd_role == 'agent', 0)
    df['idx_action'] = df.groupby(['abcd_id', 'abcd_role']).cumcount().where(df.abcd_role == 'action', 0)
    perf_log('Finished Indexing Speakers')
    
    # create metadata object in DF
    keys_to_extract = ['scenario_personal_member_level', 'scenario_order_city','scenario_flow','scenario_subflow','scenario_product_names','abcd_role']
    df = df.apply(create_metadata,args=[keys_to_extract],axis=1)
    ...
    df = df.apply(add_datetimes,axis=1,args=[start_date,period,max_abcd_id,seconds_between_utterances])
    ...
    if translation != '' and anonymize:
        raise Exception('Cannot both translate and anonymize')
    ...
    if translation != '':
        ...
        df['utterance'] = translate_text(translation, df['utterance'].to_list(),translate_client,source=source)
    else:
    ...
    if anonymize:
        ...
        perf_log("WARNING: anonymization is on - script will take several minutes to complete")
        df['original_utterance'] = df['utterance']
        df['utterance'] = df['utterance'].apply(presidio_anonymize,args=[analyzer, anonymizer, source])
    else:
    ...
    # build examples
    df = df.apply(build_example,axis=1)
...
def load_data_file(input: str) -> pandas.DataFrame:
    ...
    for i in range(len(allset)):
        try:
            ...
            pass
    perf_log("Records that couldn't have delexed removed: " + str(without_delexed))
...
def add_datetimes(row: pandas.DataFrame, start_date: datetime, period: int, max_abcd_id: int, time_per_utterance: int) -> pandas.Series:
    '''Adds a repeatable datetime based on the abcd_id'''
    created_at = start_date + timedelta(seconds=(row.idx*time_per_utterance + int(period*24*60*60*int(row.abcd_id)/max_abcd_id)))
...
def build_example(row: pandas.Series) -> pandas.Series:
    ...
    example = humanfirst.HFExample(
        ...
        context=humanfirst.HFContext(str(row.name[0]),'conversation',row['hf_role']) # abcd_id, conversation, hf_role
    )
    row['example'] = example
    return row
...
def translate_text(target: str, utterances: list, translate_client: translate.Client, source: str = None) -> str:
    ...
    if total_chars > 2:
        perf_log(f'WARNING WARNING translation exceeds google free limit: {total_chars}')
