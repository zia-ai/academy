...
def main(username: str,
    ...
    _, conversation_source_id = get_conversion_set_id(headers, namespace, convoset)
    upload_multipart(headers, namespace, conversation_source_id, filepath, upload_name, force)

def upload_multipart(headers: dict,
                     namespace: str,
    ...
    response = requests.request("POST", url, headers=headers, data=payload) # pylint: disable=missing-timeout
...
def delete_file(headers: str, namespace: str, conversation_source_id: str, filename: str) -> None:
    ...
    payload = {}
    response = requests.request("DELETE", url, headers=headers, data = payload) # pylint: disable=missing-timeout
    if response.status_code == 200:
        print(f"Deleted {filename} successfully")
    else:
        raise RuntimeError(f"Deleting existing file unsuccessful. \n Got {response.status_code} response.")
...
def get_conversion_set_id(headers: str, namespace: str, convoset: str) -> tuple:
    ...
    response = requests.request("GET", url, headers=headers,data=payload) # pylint: disable=missing-timeout
    if response.status_code != 200:
...
def authorize(username: str, password: str) -> dict:
    ...
    headers: dict
        headers containing content-type and authorization bearer token
    """
    auth_url = 'https://identitytoolkit.googleapis.com/v1/accounts:signInWithPassword?key=AIzaSyA5xZ7WCkI6X1Q2yzWHUrc70OXH5iCp7-c' # pylint: disable=line-too-long
    headers = {
    ...
    auth_response = requests.request(                                         # pylint: disable=missing-timeout
        "POST", auth_url, headers=headers, data=json.dumps(auth_body))
    if auth_response.status_code != 200:
        raise RuntimeError(f'Not authorised, google returned {auth_response.status_code} {auth_response.json()}')
