...
def process(phrases_filename: str,  intents_summary_filename: str, 
    ...
    min_threshold = min((heapq.nlargest(10,matrix.max(1))))
    intents_col = set()
    intents_val = set()
    for row in df_matrix.iterrows():
        for index in row[1].index:
            if row[1][index] >= min_threshold:
                intents_col.add(index)
                intents_val.add(row[1][index])
    print(intents_val)

    print("True positives are replaced with -1")
    print(intents_col)
    # intents_col = sorted(intents_col.items(), key=lambda x: x[1], reverse=True)
    # print(intents_col)
    # for intents in intents_col:
    #     print(intents[0])
    # quit()
    print(df_matrix.loc[filtered_labels][list(intents_col)])
    quit()
