...
def main(name: str, sample: int, workspace: str):
    ...
    df["uid"] = df["answer_annotation"].apply(humanfirst.hash_string)  
...
def create_example(row: pandas.Series, unlabelled_workspace: humanfirst.HFWorkspace, labelled_workspace: humanfirst.HFWorkspace) -> list:
    ...
    context = humanfirst.HFContext(f'convo-{row["answerid"]}','conversation', 'client')
    ...
    labelled_example = humanfirst.HFExample(
        text=row["deannotated_text"],
