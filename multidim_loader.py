...
@click.option('-o', '--load_one', is_flag=True, required=False, default=False, help='Load just the one instead of uploading all') 
...
def main(input_folder: str, prefix: str,
         ...
         delete_mode: bool, load_one: bool,
         max_files: int,
         max_loops: int) -> None: # pylint: disable=unused-argument
    """Main Function"""
    ...
    df_files_at_start = pandas.json_normalize(hf_api.list_conversation_src_files(namespace=namespace,conversation_set_src_id=convosource_id))
    ...
    for f in load_files:
        if delete_mode:
            ...
            break
        if load_one:
            multidim_data_generation.logit("Load one skip loading","")
            break
        if f in files_at_start:
            multidim_data_generation.logit("Skipping loading",f)
        else:
            
            # don't load the last file
    ...
    for f in files_at_start:
        ...
        duration_ns = end-start
        duration_s = math.ceil(duration_ns/1000000000)
        multidim_data_generation.logit(f"{duration_s:<20} Delete response is ",delete_response)
    ...
    if load_one:
        ...
        one_file_to_load = load_files.pop(-1)
        multidim_data_generation.logit(f"Starting loading just",one_file_to_load)
        start=time.perf_counter_ns()
        upload_response = hf_api.upload_json_file_to_conversation_source(namespace=namespace,
                                                        conversation_source_id=convosource_id,
                                                        upload_name=one_file_to_load,
                                                        fqfp=one_file_to_load,
                                                        )
        end=time.perf_counter_ns()
        duration_ns = end-start
        duration_s = math.ceil(duration_ns/1000000000)
...
def loop_trigger_check_until_done(hf_api: humanfirst.apis.HFAPI, max_loops: int,
                                  namespace: str, trigger_id: str, increment: int = 0, debug: bool = False) -> int:
    ...
    while done == False:
        ...
        if debug:
            multidim_data_generation.logit(f"{loops:>5} total_wait: {total_wait}",summary["status"])
