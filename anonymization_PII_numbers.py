...
def main(input: str, replace_with_0: bool) -> None:
    ...
    df["utterance_with_4_digit_between_1970_and_2030"] = df["utterances"].apply(lambda x: True if set(
        re.findall(re_4_digit, x)).intersection(set([str(i) for i in range(1970, 2031)])) else False)
    ...
    df["utterance_with_three_digit_and_cvv"] = df["utterances"].apply(lambda x: True if (
        (x.lower().find("cvv") != -1) and re.findall(re_threedigit, x)) else False)
    ...
    df.loc[df["class"] == "utterance_more_than_16_digit", "anonymized_phrases"] = df.loc[
        df["class"] == "utterance_more_than_16_digit", "utterances"].apply(replacement, args=[re_more_than_16_digit, replace_with_0])

    df.loc[df["class"] == "utterance_16_digit", "anonymized_phrases"] = df.loc[
        df["class"] == "utterance_16_digit", "utterances"].apply(replacement, args=[re_16_digit, replace_with_0])

    df.loc[df["class"] == "utterance_with_more_than_4_digit", "anonymized_phrases"] = df.loc[
        df["class"] == "utterance_with_more_than_4_digit", "utterances"].apply(replacement, args=[re_more_than_4_digit, replace_with_0])
...
def assign_class(row: pandas.Series, classes) -> str:
    ...
    if row["utterance_with_digits"] is False:
        return "no_digits"
